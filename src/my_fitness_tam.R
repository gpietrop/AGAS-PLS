...
myFitnessTAM <- function(matrix_vector) {
  n_variables <- 5
  ...
  model_string <- create_sem_model_string_from_matrix_tam(adj_matrix)
  dati_TAM <-  read.csv("ds/Data_TAM.txt", sep=";", stringsAsFactors=TRUE)
  out <- csem(.data = dati_TAM,.model = model_string, .PLS_modes = 'modeA')
  ver = verify(out)
  if (!sum(ver) == 0) {
