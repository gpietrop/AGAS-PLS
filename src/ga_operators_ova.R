...
myMutationOVA <- function(object, parent) {
  mutate <- parent <- as.vector(object@population[parent,])
  mutate_matrix <- matrix(mutate, nrow = 4, byrow = TRUE)
  ...
  if (length(indices) > 0) {
    ...
    if (length(subdiag_indices) > 0 && runif(1) <= 0.2) {
      ...
      while (length(available_indices) > 1) {
        ...
        adj_matrix <- matrix(mutate_vector, nrow = 4, byrow = TRUE)
